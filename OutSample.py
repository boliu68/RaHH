...
def OutSample_Test(A, Tr_img, Tr_tag, Tr_sim, Tst_img, Tst_qa, W, S, H_img_Tr, H_tag_Tr, gd, bit, output, parameter, imagehash_cal, imagehash):
    ...
    #[H_Al_img, qa_nouse, W_nouse, S_nousapee, Rp_Al_img, Rq_Al_img, A_img_nouse, A_qa_nouse] = initialize(Al_img, Tr_tag, img_sim, bit)
    H_Al_imgp = sign(dot(A['img'][0:rp, :], Tst_img))
    H_Al_img = hstack((H_img_Tr, H_Al_imgp))#,, H_Al_img[:, mp::]))
    Rp_Al_img = eye(Al_img.shape[1])
    Rq_Al_img = eye(Tr_tag.shape[1])
    ...
    H_Al_tagp = sign(dot(A['tag'][0:rq, :], Tst_qa))
    H_Al_tag = hstack((H_tag_Tr, H_Al_tagp))#, H_Al_tag[:, mq::]))
    Rq_Al_tag = eye(Al_tag.shape[1])
    Rp_Al_tag = eye(Tr_img.shape[1])
...
#    [H_img_nouse, H_qa_Tst, W_Tst, S_Tst] = train(Tr_img, Al_tag, H_img_Tr, H_Al_tag, S, W, tag_sim, Rp_Al_tag, Rq_Al_tag, True, 0, mq, parameter)
    ...
    GP, GR = test(sign(dot(W.transpose(), H_Al_imgp)), H_Al_tagp, gd, output) 

    
    print '-----------Out Sample CVH---------'
    print GP
    print GR
    print 'Random', sum(gd == 1) * 100.0000 / (gd.shape[0] * gd.shape[1])
